...
function getXmlData(){
	...
	ZoneName = 'Default';
	var XmlString = "<MAINCONFIG ResourceId='"+window['variable' + dynamicResourceId[pageCanvas] ]+"' MainId='"+window['variable' + dynamicMainId[pageCanvas] ]+"' MainConfigurationUserId='"+globalUserId+"' Name='"+Name+"'";
	...
	XmlString += " UserName='"+globalUserName+"' Flag='"+globalMAINCONFIG[pageCanvas].MAAINCONFIG[0].Flag+"' TftpIpAddress='' TftpImagePath='' TftpImageName='' DateTIME='"+DateTime+"'"; 
	...
	XmlString += " LinkSanityEnable='' URLConfigFlag='false' FileType='"+FileType+"' URLImageFlag='false' SetNewValues='' RedFlag='' DomainName='"+DomainName+"'";
	XmlString += " Connectivity='"+Connectivity+"' DeviceSanity='"+DeviceSanity+"' AccessSanity='"+AccessSanity+"' ZoneName='"+ZoneName+"'";
	XmlString += " DebugMode='"+window['variable' + dynamicDebug[pageCanvas] ]+"' LoadImageEnable='"+LoadImageEnable+"' LoadConfigEnable='"+LoadConfigEnable+"' SaveImageEnable='"+SaveImageEnable+"'";
	XmlString += " SaveConfigEnable='"+SaveConfigEnable+"'>";
...
/*
 *
 *  FUNCTION NAME : parentUpdateFlagChecker
 *  AUTHOR        : Mark Anthony Elbambo
 *  DATE          : January 20, 2014 
 *  MODIFIED BY   : 
 *  REVISION DATE : 
 *  REVISION #    : 
 *  DESCRIPTION   : sets the updateflag
...
function parentUpdateFlagChecker(node,level){
	...
	function rackSub(node){
		if(rackArrAC.length){
		var rackArrTemp=[], rackArrACTemp=[];
            for(var a=0; a < rackArr.length;a++){
                rackArrTemp.push(rackArr[a].ObjectPath);
            }
            for(var a=0; a < rackArrAC.length;a++){
                rackArrACTemp.push(rackArrAC[a].ObjectPath);
            }
            for(var a=0; a < rackArrACTemp.length;a++){
                for(var b=0; b < rackArrTemp.length;b++){
                    try{
                        if(rackArrTemp[b] == rackArrACTemp[a]){
                        }else{
                            upflag = "update";
                        }
                    }catch(err){
	...
	function slotSub(rackobject){
		if(slotArrAC.length){
			var sltArrTemp=[], sltArrACTemp=[];
			for(var a=0; a < slotArr.length;a++){
				sltArrTemp.push(slotArr[a].ObjectPath);
			}
			for(var a=0; a < slotArrAC.length;a++){
                sltArrACTemp.push(slotArrAC[a].ObjectPath);
            }
			for(var a=0; a < sltArrACTemp.length;a++){
				for(var b=0; b < sltArrTemp.length;b++){
					try{
						if(sltArrTemp[b] == sltArrACTemp[a]){
						}else{
							upflag = "update";
						}
					}catch(err){
	...
	function moduleSub(slot){
		if(moduleArrAC.length){
			var modArrTemp=[], modArrACTemp=[];
            for(var a=0; a < moduleArr.length;a++){
                modArrTemp.push(moduleArr[a].ObjectPath);
            }
            for(var a=0; a < moduleArrAC.length;a++){
                modArrACTemp.push(moduleArrAC[a].ObjectPath);
            }
            for(var a=0; a < modArrACTemp.length;a++){
                for(var b=0; b < modArrTemp.length;b++){
                    try{
                        if(modArrTemp[b] == modArrACTemp[a]){
                        }else{
                            upflag = "update";
                        }
                    }catch(err){
	...
	function picSub(module){
	    if(picArrAC.length){
			var picArrTemp=[], picrrACTemp=[];
            for(var a=0; a < picArr.length;a++){
                picArrTemp.push(picArr[a].ObjectPath);
            }
            for(var a=0; a < picArrAC.length;a++){
                picArrACTemp.push(picArrAC[a].ObjectPath);
            }
            for(var a=0; a < picArrACTemp.length;a++){
                for(var b=0; b < picArrTemp.length;b++){
                    try{
                        if(picArrTemp[b] == picArrACTemp[a]){
                        }else{
                            upflag = "update";
                        }
                    }catch(err){
	...
	function portSub(pic){
		if(portArrAC.length){
			var portArrTemp=[], portArrACTemp=[];
            for(var a=0; a < portArr.length;a++){
                portArrTemp.push(portArr[a].ObjectPath);
            }
            for(var a=0; a < portArrAC.length;a++){
                portArrACTemp.push(portArrAC[a].ObjectPath);
            }
            for(var a=0; a < portArrACTemp.length;a++){
                for(var b=0; b < portArrTemp.length;b++){
                    try{
                        if(portArrTemp[b] == portArrACTemp[a]){
                        }else{
                            upflag = "update";
                        }
                    }catch(err){
...
function getDeviceInformation(){
	...
	if(devicesArr.length){
		for(var t=0; t<devicesArr.length; t++){
			...
			XmlString += " HostName='"+device.DeviceName+"' UpdateFlag='"+upflag+"' MediaType='"+device.MediaType+"'";
			...
			XmlString += ">";
			if(portArrAC.length != portArr.length && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
		...
		if(devicesArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
			var XmlString2 = '';
			for(var t=0; t<devicesArrAC.length; t++){
				var device = devicesArrAC[t];
				if(devPathArr.indexOf(device.ObjectPath) == "-1"){
					XmlString2 += "<DEVICES DeviceName='"+device.DeviceName+"' DeviceType='"+device.DeviceType+"' ObjectPath='"+device.ObjectPath+"'";
...
function getDeviceInformation2(device){
	...
	if(deviceArr.length){
		for(var t=0; t<deviceArr.length; t++){
			...
			if(devObject.ObjectPath == device.ObjectPath && devObject.Page == pageCanvas){
				XmlString += "<DEVICE ChassisAddress='"+devObject.ChassisAddress+"' LoopBackAddress='' ObjectPath='"+devObject.ObjectPath+"' Username='"+devObject.Username+"'";
				XmlString += " ESXIUsername='"+devObject.ESXIUsername+"' Password='"+devObject.Password+"' ESXIPassword='"+devObject.ESXIPassword+"'";
				XmlString += " Status='"+devObject.Status+"' DeviceName='"+devObject.DeviceName+"' DevName='"+devObject.DevName+"'";
				XmlString += " DeviceId='"+devObject.DeviceId+"' RedFlag='"+devObject.RedFlag+"' ModelType='"+devObject.ModelType+"'";
...
//	if(rackArr.length && checkRackDevPath(device.ObjectPath,rackArr)){
		for(var t=0; t<rackArr.length; t++){
			...
			if(rackobject.RackDevName == device.ObjectPath && pageCanvas == rackobject.Page){
				XmlString += "<RACK RackDeviceId='"+rackobject.RackDeviceId+"' ProductNumber='"+rackobject.ProductNumber+"'";
				XmlString += " SwVersion='"+rackobject.SwVersion+"' RackName='"+rackobject.RackName+"' ObjectPath='"+rackobject.ObjectPath+"'";
				XmlString += " HwVersion='"+rackobject.HwVersion+"' Number='"+rackobject.Number+"' PortGroupSize='"+rackobject.PortGroupSize+"'";
				XmlString += " Ios='"+rackobject.Ios+"' ModuleDescription='"+rackobject.ModuleDescription+"' SerialNumber='"+rackobject.SerialNumber+"'";
				...
				XmlString += " </RACK>";
				if(rackArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
...
function getSlotObject(dev,device){
	...
	if(slotArr.length && checkSlotDevPath(device.ObjectPath,slotArr)){
		...
		for(var t=0; t<slotArr.length; t++){
			var slot = slotArr[t];
			if(slot.SlotDevName == device.ObjectPath && pageCanvas == slot.Page){
				XmlString += "<SLOT SlotDeviceId='"+slot.SlotDeviceId+"' ProductNumber='"+slot.ProductNumber+"'";
				XmlString += " SlotName='"+slot.SlotName+"' ObjectPath='"+slot.ObjectPath+"' Number='"+slot.Number+"'";
				XmlString += " ModuleDescription='"+slot.ModuleDescription+"' SerialNumber='"+slot.SerialNumber+"' ModelType='"+slot.ModelType+"'";
				XmlString += " BoardType='"+slot.BoardType+"' SlotDevName='"+slot.SlotDevName+"' SlotId='"+slot.SlotId+"'";
				...
				XmlString += " </SLOT>";
				if(slotArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
...
function getModuleObject(dev,device){
	...
	if(moduleArr.length && checkModuleDevPath(device.ObjectPath,moduleArr)){
		...
		for(var t=0; t<moduleArr.length; t++){
			...
			if(module.ModuleDevName == device.ObjectPath && pageCanvas == module.Page){
				XmlString += "<MODULE ModuleId='"+module.ModuleId+"' ModuleResId='"+module.ModuleResId+"' ModuleSlotId='"+module.ModuleSlotId+"'";
				XmlString += " Number='"+module.Number+"' ModuleDescription='"+module.ModuleDescription+"' SerialNumber='"+module.SerialNumber+"'";
				XmlString += " ModuleName='"+module.ModuleName+"' ObjectPath='"+module.ObjectPath+"' ModuleDevName='"+module.ModuleDevName+"'";
				...
				XmlString += " </MODULE>";
				if(moduleArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
...
function getPicObject(dev,device){
	...
	if(picArr.length && checkPicDevPath(device.ObjectPath,picArr)){
		...
		for(var t=0; t<picArr.length; t++){
			var pic = picArr[t];
			if(pic.PicDevName == device.ObjectPath  && pageCanvas == pic.Page){
				XmlString += "<PIC PICId='"+pic.PICId+"' PICResId='"+pic.PICResId+"' PICSlotId='"+pic.PICSlotId+"' Version='"+pic.Version+"'";
				XmlString += " PartNumber='"+pic.PartNumber+"' Number='"+pic.Number+"' Description='"+pic.Description+"'";
				XmlString += " SerialNumber='"+pic.SerialNumber+"' JedecCode='"+pic.JedecCode+"' AssemblyVersion='"+pic.AssemblyVersion+"'";
				XmlString += " AssemblyFlags='"+pic.AssemblyFlags+"' AssemblyFlags='"+pic.AssemblyFlags+"' Date='"+pic.Date+"'";
				...
				XmlString += " </PIC>";
				if(picArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
...
function getPortObject(dev,device){
	var XmlString = "";
	var objPth2 = device.ObjectPath;
	if(portArr.length && checkPortDevPath(objPth2,portArr)){
		portPathArr =[];
		for(var t=0; t<portArr.length; t++){
			var port = portArr[t];
			if(port.PortDevName == objPth2 && pageCanvas == port.Page){
				XmlString += "<PORT ObjectPath='"+port.ObjectPath+"' PortDevName='"+port.PortDevName+"' SwitchPortInfo='"+port.SwitchPortInfo+"' SwitchInfo='"+port.SwitchInfo+"'";
				XmlString += " VlanId='"+port.VlanId+"' VlanName='"+port.VlanName+"' PhysicalPortType='"+port.PhysicalPortType+"'";
				XmlString += " PortResId='"+port.PortResId+"' PortSlotId='"+port.PortSlotId+"' PortId='"+port.PortId+"'";
				...
				XmlString += "</PORT>";
				if(portArrAC.length >0 && window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
...
function getPortmapObject(dev,device){
	...
	if(lineConnected.length){
		for(var t=0; t<lineConnected.length; t++){
			var link = lineConnected[t];
			if((link.Source == dev.ObjectPath && pageCanvas == link.Page || link.Destination == dev.ObjectPath && link.Page == pageCanvas)){
				XmlString += "<PORTMAP Source='"+link.Source+"' Id='"+link.Id+"' SrcIp='"+link.SrcIp+"' DstIp='"+link.DstIp+"'";
				XmlString += " Port1Id='"+link.Port1Id+"' Port2Id='"+link.Port2Id+"' Name='"+link.Name+"' Destination='"+link.Destination+"'";
				XmlString += " LinkName='"+link.LinkName+"' CheckConnectivity='"+link.CheckConnectivity+"' ConnectivityDone='"+link.ConnectivityDone+"'";
				XmlString += " SrcMonitorSlot='"+link.SrcMonitorSlot+"' DstMonitorSlot='"+link.DstMonitorSlot+"'";
...
/*	deviceArr =[];
	...
	if(mConfig.length){
		window['variable' + dynamicResourceId[pageCanvas] ] = mConfig[0].getAttribute("ResourceId");
		window['variable' + dynamicMainId[pageCanvas] ] = mConfig[0].getAttribute("MainId");
		...
		globalMAINCONFIG[pageCanvas].MAAINCONFIG[0].Flag = mConfig[0].getAttribute("Flag");
		...
		window['variable' + dynamicDebug[pageCanvas] ] = mConfig[0].getAttribute("DebugMode");
		window['variable' + dynamicResourceId[pageCanvas] ] = checkValue(window['variable' + dynamicResourceId[pageCanvas] ]);
		window['variable' + dynamicMainId[pageCanvas] ] = checkValue(window['variable' + dynamicMainId[pageCanvas] ]);
		MainConfigurationUserId = checkValue(MainConfigurationUserId);
		Name = checkValue(Name);
		UserName = checkValue(UserName);
		globalMAINCONFIG[pageCanvas].MAAINCONFIG[0].Flag = checkValue(globalMAINCONFIG[pageCanvas].MAAINCONFIG[0].Flag);
		...
		window['variable' + dynamicDebug[pageCanvas] ] = checkValue(DebugMode);
...
function setValuesInObject(devObject,device,cnt2,myArray2){
	...
	if(window['variable' + dynamicFlagCommitted[pageCanvas] ] == true){
//		devicesArrAC=[];
//		deviceArrAC =[];
...
//				imgXPos+=50;
			...
			if(flag == true){
				...
				var myportArr  = storePortInDev(devpath);
			}else{
				setChildNodeMainDevices(device);
			}
			...
			rouletteArrPusher(devpath,model,imageObj,manu,ostype,prodfam,ipadd,protype,hostnme,"dut");
		}
	}else{
		setChildNodeMainDevices(device);
		var myportArr  = storePortInDev(devpath);
...
function setLinkInformation(){
	...
	for(var t=0; t<devicesArr.length; t++){
		if (pageCanvas == devicesArr[t].Page && devicesArr[t].Status != undefined && devicesArr[t].Status != null && devicesArr[t].Status.toLowerCase() == "reserved" && devicesArr[t].Source != "") {
